...
export class Schema {
    ...
    validateDocument(documentDom: Document, options?: ValidatorOptions): Result[] {
        ...
        if (phaseId === '#ALL') {
            return this.patterns.reduce(
                (results: Result[], pattern) =>
                    results.concat(pattern.validateDocument(documentDom, variables, fontoxpathOptions)),
                []
            )
        }

        const phase = this.phases.find(phase => phase.id === phaseId)
        const phaseVariables = Variable.reduceVariables(documentDom, phase?.variables || [], fontoxpathOptions, {
            ...variables
        })
        ...
        return (
            phase?.active
                ...
                .reduce(
                    (results: Result[], pattern) =>
                        results.concat(pattern?.validateDocument(documentDom, phaseVariables, fontoxpathOptions) || []),
                    []
                ) || []
        )
    }
    ...
    static getElementsByTagName(node: any, tagName: string) {
        ...
        function traverse(currentNode: HTMLElement) {
            ...
            for (const child of currentNode.childNodes as any) {
                traverse(child)
            }
        }
    ...
    static fromDomToJson(schematronDom: Document, options?: DomOptions): SchemaJson | Promise<SchemaJson> {
        const processIncludes = async (referenceChain: string[], schematronDom: Document) => {
            ...
            await Promise.all(
                evaluateXPathToNodes<Element>(`.//include[@href]`, schematronDom).map(async includeElement => {
                    const href = includeElement.getAttribute('href')!
                    const includedDom = parseXmlDocument(await options.fetchReference(href, referenceChain))
                    ...
                    includeElement.parentNode?.insertBefore(includedDom.documentElement, includeElement)
        ...
        if (options) {
            return processIncludes([], schematronDom).then(() => Schema.fromDomToJson(schematronDom))
        }
