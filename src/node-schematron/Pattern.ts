...
export class Pattern {
    ...
    validateDocument(documentDom: Document, parentVariables: object | null, fontoxpathOptions: FontoxpathOptions) {
        ...
        const ruleContexts = this.rules.map(rule =>
            evaluateXPathToNodes<Node>('//(' + rule.context + ')', documentDom, null, variables, fontoxpathOptions)
        )
        const flattenValidationResults = (results: Result[], node: Node): Result[] => {
            ...
            if (rule) {
                results.splice(results.length, 0, ...rule.validateNode(node, variables, fontoxpathOptions))
            }
